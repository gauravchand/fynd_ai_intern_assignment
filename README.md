...
🔗 **Live Demo:** [Insert Your Hugging Face Space URL Here]
...
```text
├── task1_rating_prediction.ipynb  # Task 1: Jupyter Notebook with Prompt Experiments
├── app.py                         # Task 2: Main Streamlit Application Code
├── requirements.txt               # Dependencies
├── results_zero_shot_groq.csv     # Results from Zero-Shot experiment
├── results_few_shot_groq.csv      # Results from Few-Shot experiment
├── results_cot_groq.csv           # Results from Chain-of-Thought experiment
...
Few-Shot: Provided 3 examples (Negative, Neutral, Positive) to guide the model.

Chain-of-Thought (CoT): Forced the model to analyze keywords and tone before assigning a rating.
...
3. Set Up API Key
You need a Groq API key. You can set it in your environment or rely on the secrets management.
...
Data Storage: Uses a local reviews.csv for data persistence. (Note: On the free tier, this file resets when the Space restarts).
